...
def test_solver_init():
    ...
    assert instance.pacman_solution_file == "./solutions/test/test_pacman_solution.txt"
    assert instance.highest_score_file == "./worlds/test/pacman_test.txt"


def test_manhattan_distance():
    instance = solver.Solver('config/test_run_config.json')
    distance = instance._calculate_manhattan_distance((0, 0), (20, 20))
    assert distance == 40


def test_manhattan_distance_1():
    instance = solver.Solver('config/test_run_config.json')
    distance = instance._calculate_manhattan_distance((10, 0), (15, 20))
    assert distance == 25
...
def test_generate_sensor_inputs():
    ...
    sensor_inputs = instance._generate_sensor_inputs([0, 0], gpac.PACMAN)
    assert sensor_inputs == [53, 1, 0, 36, 55]
...
def test_select_best_move():
    ...
    assert solver.Solver._select_best_move(move_scores, gpac.PACMAN) == 'down'
...
def test_crossover():
    ...
    assert child.children[0].children[0].data == 'F'
    assert child.children[0].children[1].data == 'P'
    assert child.children[1].data == 'RAND'
    assert child.children[1].children[0].data == 'W'
    assert child.children[1].children[1].data == 'F'
...
def test_mutate():
    ...
    assert child.data == '*'
    assert child.children[0].data == '/'
    assert child.children[0].children[0].data == 1.2
