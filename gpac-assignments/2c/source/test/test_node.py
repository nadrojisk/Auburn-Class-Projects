...
def test_init():
    random.seed(2)
    instance = node.Node(10, max_depth=10)
...
def test_grow_max_depth():
    instance = node.Node(9, max_depth=9)
    instance.grow()
    assert instance.children == [None, None]
...
def test_grow_almost_max():
    random.seed(3)
    instance = node.Node(max_depth=1)
    instance.grow()

    assert instance.children[0].data == 0.8845845059190367
...
def test_grow_twice():
    random.seed(3)
    instance = node.Node(max_depth=2)
    instance.grow()
...
def test_get_value_from_tree():
    ...
    value = instance.calculate_pacman(10, 1, 2, 0, 0)
...
def test_height_top():
    random.seed(4)
    instance = node.Node(0, max_depth=1, tree_type='full')
    instance.grow()
