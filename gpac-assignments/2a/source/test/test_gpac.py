...
def test_place_units():
    instance = gpac.GPac('maps/map0.txt', .50, .2, 0, 10)

    assert instance.board[0][0] == gpac.PACMAN
    assert instance.board[-1][-1] in gpac.GHOST


def test_redraw_pills_fruits():
    instance = gpac.GPac('maps/map0.txt', .50, 1, 0, 10)
    instance.locations[gpac.PILL].append([-1, -1])

    assert gpac.PILL != instance.board[-1][-1]

    instance.locations[gpac.GHOST[0]] = [-1, -2]
    instance.board[-1][-2] = gpac.GHOST[0]
    instance.board[-1][-1] = gpac.EMPTY_CELL
    instance.locations[gpac.GHOST[1]] = [-1, -2]
    instance.board[-1][-2] = gpac.GHOST[1]
    instance.locations[gpac.GHOST[2]] = [-1, -2]
    instance.board[-1][-2] = gpac.GHOST[2]

    instance._redraw_pills_fruits()
    assert gpac.PILL == instance.board[-1][-1]
...
def test_move_pacman():
    instance = gpac.GPac('maps/map0.txt', .50, .5, 0, 10)
    assert instance.board[0][0] == gpac.PACMAN
    assert instance.board[1][0] != gpac.PACMAN
...
def test_score_calculation_all_but_one_pill():
    ...
    assert instance._calculate_score() == 99
